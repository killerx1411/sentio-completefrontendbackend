GET_ALL_USERS = """
    SELECT u.id, u.full_name, u.email, u.status, u.registration_status, u.phone, u.department,
           ...
           u.is_first_login, u.temp_password_expiry, u.assigned_school, u.assigned_class,
...
UPDATE_USER = """
    ...
    SET full_name = %s, email = %s, status = %s, phone = %s, department = %s, employee_id = %s,
        mfa_enabled = %s, profile_image = %s, assigned_school = %s, assigned_class = %s, updated_by = %s
    WHERE id = %s
    RETURNING id, full_name, email, status, registration_status, phone, department, employee_id,
              mfa_enabled, profile_image, assigned_school, assigned_class, updated_at;
"""

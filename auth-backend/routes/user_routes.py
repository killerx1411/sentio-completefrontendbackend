...
def create_user():
    ...
    return success_response(data=result["data"], message="User created successfully", status_code=201)
...
def update_user(user_id):
    ...
    role_id = data.get("role_id")
    role_name = get_role_name(role_id)
    if role_name in ("Super Admin", "Secondary Admin", "Normal Admin") and not is_current_super:
    ...
    if "error" in result:
        return error_response(result["error"], result.get("status", 400))
    return success_response(data=result["data"], message="User updated successfully")
...
def get_dashboard_stats():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) AS count FROM auth_enabler.audit_logs WHERE action = 'LOGIN';")
